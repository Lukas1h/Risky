...
function render(){
    ...
    islandNode = rc.path(island, {roughness: 1.2,fillStyle: 'cross-hatch',fill:"rgba(0,0,0,0.1)",combineNestedSvgPaths: true,seed:1})
    islandNode.setAttribute("transform", "scale(3) translate(-20,-80)")
    ...
    pieces.forEach((pieceInfo)=>{
        ...
        const divMove = (e)=>{
            ...
            var transformAttr = ' translate(' + (e.clientX+50) + ',' + (e.clientY+50) + ')';

            planeNode.setAttribute('transform', transformAttr + "rotate(180)");
            pieces[pieceInfo.index].pos.x = e.clientX
            pieces[pieceInfo.index].pos.y = e.clientY

        }

        planeNode.addEventListener('mousedown', ()=>{
            clickedPlaneFirst = true
...
function moveSection(xOffset, yOffset) {
    ...
    var planeNode = rc.path(plane, {roughness: 1.2,fillStyle: 'solid',fill:"rgba(0,0,0,0.1)",combineNestedSvgPaths: true,fillWeight: 1,seed:1});

    var transformAttr = ' translate(' + (xOffset+50) + ',' + (yOffset+50) + ')';
    planeNode.setAttribute('transform', transformAttr + "rotate(180)");
